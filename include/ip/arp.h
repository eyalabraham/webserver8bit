...
hwaddr_t* const arp_query(struct net_interface_t* const,    // search the table for HW address of an IP address
                          ip4_addr_t);
ip4_err_t arp_add_entry(struct net_interface_t* const,      // add an ARP entry to the table
                        ip4_addr_t,
                        hwaddr_t,
                        arp_flags_t);
ip4_err_t arp_update_entry(struct net_interface_t* const,   // update an ARP entry in the table
                           ip4_addr_t,
                           hwaddr_t,
                           arp_flags_t);
struct arp_tbl_t* const arp_show(void);                     // repeated calls to this function will walk the ARP table

void arp_packet_handler(struct pbuf_t* const,               // link layer packet handler for handling incoming ARP or forwarding
