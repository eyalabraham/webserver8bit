...
#define     PPIPC_INIT      0x87                // PC0..PC2 initialization, INTE1 and INTE2 set with 8255 bit-clear command
...
#define     INTE1_SET       outp(PPICNT,0x0d)   // INTE flip-flop, IBF, and OBF control
#define     INTE1_CLEAR     outp(PPICNT,0x0c)
#define     INTE2_SET       outp(PPICNT,0x09)
#define     INTE2_CLEAR     outp(PPICNT,0x08)
#define     IBF_CLEAR       outp(PPICNT,0x0a)
#define     OBF_SET         outp(PPICNT,0x0f)


// NEC v25 interrupt and IO definitions
 ...
 *
 */
#define     LOOP    		45000               // this will be approx 170mSec
...
//    INTE2_CLEAR;
    ...
    for ( i = 0; i < LOOP; i++);                // wait at least 150mSec, which is also needed by LCD to reset
 ...
 * spiWriteBlock()
 *
 *  write a block of data from buffer location of certain size,
 *  at completion call a callback function
 *
 */
int spiWriteBlock(spiDevice_t device, unsigned char* outputBuffer, unsigned int count, void (*spiCallBack)(void))
{
    ...
    if ( activeDevice == NONE )
    {
        ...
        INTE1_SET;                              // setup 8255 to generate interrupt on mode-2 outputs
                                                ...
                                                // pointer to 20-bit linear DMA address
        dwLinearAddress = (unsigned long) FP_SEG(outputBuffer) * 16 + (unsigned long) FP_OFF(outputBuffer);
